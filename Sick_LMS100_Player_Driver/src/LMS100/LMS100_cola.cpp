 ...
 * - 2 marked with >>>
 */
lms100_cola::lms100_cola(const char* host, int port, int debug_mode, bool upside_down_mode) {
	// init variables
	portno 			= port;
	hostname 		= host;
	debug 			= debug_mode;
...
int lms100_cola::SetResolutionAndFrequency(float freq, float ang_res, float angle_start, float angle_stop) {
	...
	if (error == 0) {
		...
		// Check angle_range -1 because of segmentation fault
		if (((angle_range) < 1) || ((angle_range) > LMS100_MAXRANGE-1)) {
...
player_laser_data_t lms100_cola::ReadMeasurement() {
	...
	if ((playerConfig.resolution * 100) == 50) {
		...
		for (uint32_t i = 0; i < playerData.ranges_count; i++) {
			...
			playerData.ranges[i] = (((float) vbuffer[26 + (starting) + j])
					/ measHeader.ScalingFactor);
			if(playerData.ranges[i]==0 ){
				playerData.ranges[i]=playerConfig.max_range;
			}
			if (debug == 2)
...
int lms100_cola::ReceiveResults() {
	...
	while (true) {
		...
		if (n < 0) {
			...
			std::cout << "One more Time - DAFT PUNK!!!" << std::endl;
//			return (-1);
		}
