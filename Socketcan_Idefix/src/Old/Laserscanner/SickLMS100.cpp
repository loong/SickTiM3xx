...
SickLMS100::SickLMS100(ConfigFile* cf, int section) :
	...
	angular_resolution = cf->ReadFloat(section, "angular_resolution",
			DEFAULT_LMS100_ANGULAR_RES);
	scanning_frequency = cf->ReadFloat(section, "scanning_frequency",
			DEFAULT_LMS100_FREQUENCY);
...
int SickLMS100::MainSetup() {
	...
	std::cout << "SickLMS100::MainSetup()" << std::endl;

	// Create a lms100_cola object
	lms100 = new lms100_cola(hostname, port, debug, upside_down);
...
int SickLMS100::ChangeUserlevel() {
	if (strncmp(password, "NULL", 4) != 0) {
		...
		if (strncmp(password, (const char*) DEFAULT_LMS100_L2_PASSWORD, 8) == 0)
			userlevel = 2;
		else if (strncmp(password, (const char*) DEFAULT_LMS100_L3_PASSWORD, 8)
				== 0)
			userlevel = 3;
		else if (strncmp(password, (const char*) DEFAULT_LMS100_L4_PASSWORD, 8)
				== 0)
			userlevel = 4;
		else {
		...
		if (lms100->SelectUserLevel(userlevel, password) != 0) {
			PLAYER_WARN2(
					"> Change to userlevel %d using password %s... [failed]",
					userlevel, password);
			return -1;
		} else {
			logged_in = true;
 ...
 */
int SickLMS100::ProcessMessage(QueuePointer &resp_queue, player_msghdr* hdr,
		void* data) {
	...
	// Set Configuration
		...
		if (ChangeUserlevel() != -1) {
			if (userlevel > 2) {
				// Set scanning parameters
				if (lms100->SetResolutionAndFrequency(scanning_frequency,
						angular_resolution, min_angle, max_angle - min_angle)
						!= 0)
					PLAYER_WARN(
							"> Couldn't configure resolution, frequency, and min/max angle. Using previously set values... [failed]");
				else
